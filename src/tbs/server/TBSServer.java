 ...
 * Tickets are issued for _seatList.
 * Seats in theatres are arranged as a square block, with as many _seatList in a row as there are rows.
 * A seat is identified by the row number it is in and its position in the row. 
 * About half the _seatList are considered "premium" seating. These are the lowest numbered rows.
...
public interface TBSServer {
	 ...
	 * the floor area (square metres). All theatres have as many _seatList in a row as they do rows, and the number of
	 ...
	 * The premium price applies to the _seatList in the rows closest to the stage, which in this case
	 * are rows 1 to floor(Number of Rows/2). The remaining _seatList have the cheap seat price.
	 ...
	 * @param premiumPriceStr The price for the premium _seatList. It must be in the format $d where &lt;d&gt; is the number of dollars.
	 * @param cheapSeatsStr The price for the cheap _seatList. It must be in the format $d where &lt;d&gt; is the number of dollars.
	...
	/**
	 * Request the server to supply a list of the _seatList that are still available for the performance with
	 * the specified ID.
	 * <p>The request fails if there is no performance with the specified ID.
	 * @param performanceID The ID for the performance to determine the available _seatList for.
